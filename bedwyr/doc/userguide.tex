...
and \verb.D. for disproved. Generic variables are not explicitely 
quantified but represented as \verb.n1., \verb.n2., etc. The formulas are 
abstracted over their universal variables. The relative scopings
of generic universal variables is not represented,
although that information is present internally,
to avoid that a proof of $(\forall x\nabla y\qs p~x~y)$
is used as a proof for $(\nabla y\forall x\qs p~x~y)$. This will 
be naturally fixed with planned extensions of the tabling mechanisms
implicitely allowing extra axioms on nabla (see \cite{tiu06lfmtp}),
in order to be able to inspect in a meaningful way one predicate's table from 
another logic program.
...
A naive and wrong implementation would proceed as follows:
Try to prove \verb.p., unfold it, try \verb.q., unfold it, try \verb.p..
Noticing a loop, a failure would happen, resulting in the marking of \verb.q.
as disproved. Then the second clause for \verb.p. is tried, and yields a 
success. In the end, it'd have marked \verb.q. as false, which is wrong.

To avoid this kind of problem, Bedwyr is very conservative and marks goals as 
