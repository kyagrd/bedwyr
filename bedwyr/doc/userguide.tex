...
predicates that have been proven or disproven, hence avoiding redundant search.
Bedwyr uses tabling in the underlying implementation to keep track
of proven or disproven predicates.
...
we would have a proof of $p\,t$. Bedwyr keeps both previously proven goals 
(e.g., coinductive $p\,t$ in the previous example), 
or disproven goals (e.g., inductive $p\,t$). 

Tabling is by default not enabled in Bedwyr. Bedwyr provides to
keywords: \texttt{inductive} and \texttt{coinductive}, to explicitly
enable tabling for a predicate. 
...
Tabling is used in both Prover Right and Prover Left.
In the current implementation of Bedwyr, only ground atomic predicates
are tabled. Note that ``ground predicates'' here have different meaning
depending on whether they occur in Prover Right or Prover Left.
...
%% Alwen: we need a simpler example to illustrate tabling.
...
prover), tabling, and depth-first search.  
