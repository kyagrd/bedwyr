...
struct
  ...
  ********************************************************************)
  ...
  (********************************************************************
  *andThenTactical:
  * Applies the given tactical.  Then pairwise applies the given
  * tacticals to the newly created sequents.
  *
  * The new sequents are those returned by 
  ********************************************************************)
  let rec andThenTactical tac tacs =
    fun sequents sc fc ->
      (*  Build a new builder from a previous builder and the current
          builder.  *)
      let newBuilder oldbuilder builder newseqs =
        fun proofs ->
          let l = (List.length proofs) - (List.length newseqs) in
          let (potherseqs, pseqs) = Listutils.split_nth l proofs in
          (oldbuilder potherseqs) @ (builder pseqs)
      in
      
      (****************************************************************
      *scFirst:
      * Success continuation for the result of the first tactical;
      * just starts pairwise application of the list of tacticals
      * to the list of new sequents.
      ****************************************************************)
      let rec scFirst =
        fun newseqs oldseqs builder k ->
          match (tacs, newseqs) with
            | (_, []) -> sc newseqs oldseqs builder k
            | ([], s::ss) -> idTactical [s] (scRest builder [] ss [] oldseqs) k
            | (tac::tacs', s::ss) -> tac [s] (scRest builder tacs' ss [] oldseqs) k
      
      (****************************************************************
      *scRest:
      * Continues pairwise applicaiton of the list of tacitcals to the
      * list of new sequents generated by the original tactical; doesn't
      * do anything to the sequents generated by those tacticals in the
      * list.
      ****************************************************************)
      and scRest oldbuilder tacs rest realnewseqs realoldseqs =
        fun newseqs oldseqs builder k ->
          let newseqs' = newseqs @ oldseqs in
          let realnewseqs' = realnewseqs @ newseqs' in
          let builder' = newBuilder oldbuilder builder newseqs' in
          match (tacs, rest) with
            | (_, []) -> sc realnewseqs' realoldseqs builder k
            | ([], s::ss) ->
                idTactical [s] (scRest builder' [] ss realnewseqs' realoldseqs) k
            | (tac::tacs', s::ss) ->
                tac [s] (scRest builder' tacs' ss realnewseqs' realoldseqs) k
      in

      match (tacs, sequents) with
        ([], _) -> fc ()
      | (_, []) -> sc [] [] idProofBuilder fc
      | (_, s::ss) -> (tac [s] scFirst fc)
  ...
  ********************************************************************) 
  ...
  let rec andThenInterface session args = match args with
      [Absyn.Tactical(tac)] -> tac
    | Absyn.Tactical(tac)::tacs ->
        let get tacs = match tacs with
            Absyn.Tactical(tac) -> Some tac
          | _ -> None
        in
        let tacs' = List.map get tacs in
        if List.exists Option.isNone tacs' then
          invalidArguments "andthen"
        else
          andThenTactical tac (List.map Option.get tacs')
    | _ -> invalidArguments "andthen"
  ...
  let tacticals =
    ...
    let ts = Table.add "andthen" andThenInterface ts in
