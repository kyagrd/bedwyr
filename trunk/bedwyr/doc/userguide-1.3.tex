...
\title{{\Huge A User Guide to Bedwyr v1.3}
   \thanks{Support has been obtained for this work from the following
           ...
           Associ{\'e}es'' Slimmer, and now the ``ADT'' BATT),
...
\author{David Baelde$^1$, Andrew Gacek$^2$, Dale Miller$^1$, \\
        Gopalan Nadathur$^2$, Alwen Tiu$^3$, Quentin Heath$^1$  \\ \\
...
right-associative, to mimic the behaviour of {\tt ->}.
...
The first argument (\texttt{Agent : (p -> p -> prop)}) was
systematically removed. See the example file for the full definition.}
\label{one-step}
...
Define coinductive bisim : p -> p -> prop by
  bisim P Q :=
    ...
    (forall X M, onep P (dn X) M ->
      exists N, onep Q (dn X) N /\ forall w, bisim (M w) (N w)) /\
    (forall X M, onep P (up X) M ->
      exists N, onep Q (up X) N /\ nabla w, bisim (M w) (N w)) /\
    (forall A Q1, one Q A Q1 ->
      exists P1, one P A P1 /\ bisim P1 Q1) /\
    (forall X N, onep Q (dn X) N ->
      exists M, onep P (dn X) M /\ forall w, bisim (M w) (N w)) /\
    (forall X N, onep Q (up X) N ->
      exists M, onep P (up X) M /\ nabla w, bisim (M w) (N w)).
\end{verbatim}
\caption{The definition of (open) bisimulation. See remark under
Figure~\ref{one-step}.}
\label{bisim}
\end{figure}
...
[...welcome message...]
?= bisim dummy_agent (in a x\ in a y\ z)
     (in a x\ nu w\ in a y\ out w w z).
Yes.
More [y] ? y
No more solutions.
?= bisim dummy_agent (in a x\ nu y\ match x y (out c c z))
     (in a x\ z).
Yes.
More [y] ? y
No more solutions.
?= bisim dummy_agent
     (nu x\ out a x (in c y\ match x y (out c c z)))
