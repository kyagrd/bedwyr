...
noticing. To solve both issues, we use tabling.

For tabled definitions, being inductive or co-inductive matters, and this 
...
Tabling is used in both provers.
In Prover Right, tabling only applies to goals which do not contain logic 
variables. In Prover Left, tabled goals cannot contain logic variables
\emph{or} eigenvariables.
...
contents of \verb.pred.'s table: formulas which have been proven or
disproven.  In some cases this information has important uses: for
...
predicate {\tt bisim} describes a bisimulation.  For example,
\begin{verbatim}
> bedwyr examples/pi_nat.def
[..welcome message..]
...
[..]
\end{verbatim}
In this list, formulas are marked as proven or disproven. The
...
The strategy used by Bedwyr for attempting proofs in not complete.
That strategy involves using two provers (the left and the right
prover), tabling, and depth-first search.  While many interesting
formulas can be proved with this strategy, it is certainly
incomplete.  Many of the incompleteness that one encounters in
...
decidable, uniary, and did not need typing information.  This subset
...
Bedwyr will ocassionally complain that it needs to solve a ``not LL
...
into the abstraction {\tt P} (Bedwyr will do $\beta$-reduction inorder
...
\subsubsection{Logic variables from the right prover in the left prover}

As we have already noted, logic variables used in the implementation
of proof search in the right prover are not allowed in the left
prover.  This restriction arises more from the fact that we do not
have a satisfactory and comprehensive understanding of unification in
the left prover that incorporates such variables.  As a result, Bedwyr
...
run-time error since it requires dealing with a right-prover logic
variable within left-prover unification.  The second query illustrates
that some instances of the left-prover unification tolerates
occurrences of right-prover logic variables.
